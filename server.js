...
const MEETING_FILES_DIR = path.join(UPLOADS_DIR, 'files');
if (!fs.existsSync(MEETING_FILES_DIR)) fs.mkdirSync(MEETING_FILES_DIR, { recursive: true });
...
const fileUpload = multer({
  storage: multer.diskStorage({
    destination: (_req, _file, cb) => cb(null, MEETING_FILES_DIR),
    filename: (_req, file, cb) => cb(null, `${crypto.randomUUID()}-${file.originalname.slice(0, 100)}`),
  }),
  limits: { fileSize: 50 * 1024 * 1024 }, // 50 MB max
  fileFilter: (_req, file, cb) => {
    const allowed = ['image/', 'application/pdf', 'text/', 'application/msword',
      'application/vnd.openxmlformats', 'application/vnd.ms-excel', 'application/vnd.ms-powerpoint',
...
function createMeetingState(id, adminToken, title, settings, ownerId, ownerCompanyId) {
  return {
    id, adminToken, title,
    createdAt: Date.now(),
...
function activateScheduledMeeting(scheduled) {
  const meeting = createMeetingState(
    scheduled.id, scheduled.adminToken, scheduled.title,
    scheduled.settings, scheduled.ownerId, scheduled.ownerCompanyId
  );
...
const recurringMeetingPoller = setInterval(async () => {
  try {
    const { rows } = await pool.query(
      `SELECT * FROM recurring_meetings WHERE is_active = TRUE AND next_occurrence <= NOW()`
    );
    for (const rm of rows) {
      const existingMeeting = meetings.get(rm.meeting_id);
      if (!existingMeeting) {
        const settings = typeof rm.settings === 'string' ? JSON.parse(rm.settings) : (rm.settings || {});
        const meeting = createMeetingState(
          rm.meeting_id, rm.admin_token, rm.title,
          { muteOnJoin: settings.muteOnJoin ?? false, videoOffOnJoin: settings.videoOffOnJoin ?? false,
            maxParticipants: settings.maxParticipants ?? 50, locked: false, waitingRoom: settings.waitingRoom ?? false },
          rm.user_id, rm.company_id
        );
        meetings.set(rm.meeting_id, meeting);
        // Persist to meetings_log
        pool.query(
          `INSERT INTO meetings_log (meeting_id, title, user_id, company_id) VALUES ($1,$2,$3,$4) RETURNING id`,
          [rm.meeting_id, rm.title, rm.user_id, rm.company_id]
        ).then(({ rows: logRows }) => { meeting.logId = logRows[0].id; }).catch(err => console.error('recurring log error:', err.message));
      }
      // Compute and update next occurrence
    ...
    console.error('recurringMeetingPoller error:', err.message);
  }
...
function computeNextOccurrence(recurrence, dayOfWeek, dayOfMonth, timeUtc) {
  ...
  if (recurrence === 'daily') {
    ...
    const targetDay = dayOfWeek ?? 1;
    const diff = (targetDay - next.getUTCDay() + 7) % 7;
    next.setUTCDate(next.getUTCDate() + (diff === 0 && next <= now ? 14 : diff));
...
async function removeParticipantFromMeeting(meeting, participantId, io) {
  ...
  const attendanceRecord = meeting.attendance.findLast(a => a.participantId === participantId && !a.leftAt);
  if (attendanceRecord) attendanceRecord.leftAt = Date.now();
...
app.post('/api/meetings/guest', guestMeetingLimiter, async (req, res) => {
  ...
  const title      = ((req.body.title || 'Untitled Meeting') + '').slice(0, 100);
  const settings   = { muteOnJoin: false, videoOffOnJoin: false, maxParticipants: 50, locked: false, waitingRoom: false };
  const meeting    = createMeetingState(id, adminToken, title, settings, null, null);
...
app.post('/api/meetings', authApi, async (req, res) => {
  ...
  if (req.body.templateId) {
    try {
      ...
      if (tplRows.length) {
        const tplSettings = tplRows[0].settings;
        if (tplSettings.muteOnJoin !== undefined) settings.muteOnJoin = tplSettings.muteOnJoin;
        if (tplSettings.videoOffOnJoin !== undefined) settings.videoOffOnJoin = tplSettings.videoOffOnJoin;
        if (tplSettings.maxParticipants !== undefined) settings.maxParticipants = Math.min(Math.max(tplSettings.maxParticipants, 2), 500);
        if (tplSettings.waitingRoom !== undefined) settings.waitingRoom = tplSettings.waitingRoom;
      }
...
app.post('/api/meetings/:meetingId/polls', authApi, findMeeting, requireMeetingAdmin, (req, res) => {
  ...
  if (!question || !Array.isArray(options) || options.length < 2 || options.length > 10) {
    return res.status(400).json({ error: 'Provide question and 2-10 options' });
  }
  const pollId = crypto.randomUUID();
  const poll = {
    id: pollId,
    question: (question + '').trim().slice(0, 500),
    options: options.slice(0, 10).map((o, i) => ({ id: `opt_${i}`, text: (o + '').trim().slice(0, 200) })),
    votes: new Map(),
    isActive: true,
    createdBy: 'admin',
    createdAt: Date.now(),
  ...
  io.to(req.meeting.id).emit('poll:created', {
    id: poll.id, question: poll.question, options: poll.options, isActive: true,
  });
...
app.post('/api/meetings/:meetingId/polls/:pollId/vote', authApi, findMeeting, (req, res) => {
  ...
  const voterId = participantId || req.apiUser?.userId?.toString() || 'api';
...
app.get('/api/meetings/:meetingId/polls', authApi, findMeeting, (req, res) => {
  const polls = [...req.meeting.polls.values()].map(p => ({
    id: p.id, question: p.question, options: p.options, isActive: p.isActive,
...
app.post('/api/meetings/:meetingId/questions', authApi, findMeeting, (req, res) => {
  ...
  const question = {
    ...
    askedBy: { name: (participantName || 'Anonymous').slice(0, 60) },
    upvotes: new Set(),
    isAnswered: false,
    answer: null,
    answeredBy: null,
  ...
  io.to(req.meeting.id).emit('qa:new', {
    id: question.id, text: question.text, askedBy: question.askedBy,
    upvoteCount: 0, isAnswered: false, createdAt: question.createdAt,
  });
...
app.post('/api/meetings/:meetingId/questions/:questionId/upvote', authApi, findMeeting, (req, res) => {
  ...
  if (q.upvotes.has(voterId)) q.upvotes.delete(voterId);
  else q.upvotes.add(voterId);
...
app.post('/api/meetings/:meetingId/questions/:questionId/answer', authApi, findMeeting, requireMeetingAdmin, (req, res) => {
  ...
  if (!q) return res.status(404).json({ error: 'Question not found' });
  q.isAnswered = true;
  q.answer = (req.body.answer || '').slice(0, 1000);
  q.answeredBy = (req.body.answeredBy || 'Host').slice(0, 60);
  io.to(req.meeting.id).emit('qa:updated', {
    questionId: q.id, isAnswered: true, answer: q.answer, answeredBy: q.answeredBy,
  });
...
app.get('/api/meetings/:meetingId/questions', authApi, findMeeting, (req, res) => {
  const questions = [...req.meeting.questions.values()]
    .filter(q => !q.dismissed)
    .map(q => ({
      id: q.id, text: q.text, askedBy: q.askedBy, upvoteCount: q.upvotes.size,
      isAnswered: q.isAnswered, answer: q.answer, answeredBy: q.answeredBy, createdAt: q.createdAt,
    }))
...
// ─── Breakout Rooms ───────────────────────────────────────────────────────────
app.post('/api/meetings/:meetingId/breakout-rooms', authApi, findMeeting, requireMeetingAdmin, (req, res) => {
  const { rooms } = req.body;
  if (!Array.isArray(rooms) || rooms.length < 1 || rooms.length > 50) {
    return res.status(400).json({ error: 'Provide 1-50 room names' });
  }
  req.meeting.breakoutRooms.clear();
  for (const r of rooms.slice(0, 50)) {
    const roomId = crypto.randomUUID();
    req.meeting.breakoutRooms.set(roomId, {
      id: roomId, name: ((r.name || r) + '').trim().slice(0, 100) || `Room ${req.meeting.breakoutRooms.size + 1}`,
      participants: new Set(),
    });
  }
  const roomList = [...req.meeting.breakoutRooms.values()].map(r => ({ id: r.id, name: r.name, participants: [...r.participants] }));
  io.to(req.meeting.id).emit('breakout:updated', { rooms: roomList, isOpen: req.meeting.breakoutOpen });
  trackEvent(req.apiUser?.userId, req.apiUser?.companyId, 'feature.breakout_rooms', { action: 'create', count: rooms.length });
  res.status(201).json({ rooms: roomList });
});

app.patch('/api/meetings/:meetingId/breakout-rooms', authApi, findMeeting, requireMeetingAdmin, (req, res) => {
  const { assignments } = req.body;
  if (!assignments || typeof assignments !== 'object') {
    return res.status(400).json({ error: 'assignments object required' });
  }
  for (const room of req.meeting.breakoutRooms.values()) room.participants.clear();
  for (const [roomId, pids] of Object.entries(assignments)) {
    const room = req.meeting.breakoutRooms.get(roomId);
    if (room && Array.isArray(pids)) {
      for (const pid of pids) room.participants.add(pid);
    }
  }
  const roomList = [...req.meeting.breakoutRooms.values()].map(r => ({ id: r.id, name: r.name, participants: [...r.participants] }));
  io.to(req.meeting.id).emit('breakout:updated', { rooms: roomList, isOpen: req.meeting.breakoutOpen });
...
app.post('/api/meetings/:meetingId/breakout-rooms/open', authApi, findMeeting, requireMeetingAdmin, (req, res) => {
  req.meeting.breakoutOpen = true;
  const timerMinutes = parseInt(req.body.timerMinutes) || 0;
  if (timerMinutes > 0) {
    if (req.meeting.breakoutTimer?.timerId) clearTimeout(req.meeting.breakoutTimer.timerId);
    const endsAt = Date.now() + timerMinutes * 60000;
    const mid = req.meeting.id;
    req.meeting.breakoutTimer = {
      endsAt,
      timerId: setTimeout(() => {
        const m = meetings.get(mid);
        if (!m) return;
        m.breakoutOpen = false;
        m.breakoutTimer = null;
        for (const room of m.breakoutRooms.values()) room.participants.clear();
        io.to(mid).emit('breakout:closed', { reason: 'Timer expired' });
      }, timerMinutes * 60000),
    };
  }
  for (const room of req.meeting.breakoutRooms.values()) {
    for (const pid of room.participants) {
      const p = req.meeting.participants.get(pid);
      if (p) io.to(p.socketId).emit('breakout:move', { roomId: room.id, roomName: room.name });
    }
  }
  const roomList = [...req.meeting.breakoutRooms.values()].map(r => ({ id: r.id, name: r.name, participants: [...r.participants] }));
  io.to(req.meeting.id).emit('breakout:updated', { rooms: roomList, isOpen: true, timerEndsAt: req.meeting.breakoutTimer?.endsAt || null });
  res.json({ opened: true, timerEndsAt: req.meeting.breakoutTimer?.endsAt || null });
});

app.post('/api/meetings/:meetingId/breakout-rooms/close', authApi, findMeeting, requireMeetingAdmin, (req, res) => {
  req.meeting.breakoutOpen = false;
  if (req.meeting.breakoutTimer?.timerId) clearTimeout(req.meeting.breakoutTimer.timerId);
  req.meeting.breakoutTimer = null;
  for (const room of req.meeting.breakoutRooms.values()) room.participants.clear();
  io.to(req.meeting.id).emit('breakout:closed', { reason: 'Closed by host' });
  res.json({ closed: true });
});

app.post('/api/meetings/:meetingId/breakout-rooms/broadcast', authApi, findMeeting, requireMeetingAdmin, (req, res) => {
  const { text } = req.body;
  if (!text || typeof text !== 'string') return res.status(400).json({ error: 'text required' });
  io.to(req.meeting.id).emit('breakout:message', { text: text.trim().slice(0, 500), from: 'Host' });
  res.json({ sent: true });
});

app.get('/api/meetings/:meetingId/breakout-rooms', authApi, findMeeting, (req, res) => {
  const rooms = [...req.meeting.breakoutRooms.values()].map(r => ({ id: r.id, name: r.name, participants: [...r.participants] }));
  res.json({ rooms, isOpen: req.meeting.breakoutOpen, timerEndsAt: req.meeting.breakoutTimer?.endsAt || null });
});
...
app.put('/api/meetings/:meetingId/notes', authApi, findMeeting, requireMeetingAdmin, (req, res) => {
  ...
  req.meeting.notes.content = content.slice(0, 50000);
  req.meeting.notes.lastUpdatedBy = 'Host';
  req.meeting.notes.lastUpdatedAt = Date.now();
  io.to(req.meeting.id).emit('notes:updated', { content: req.meeting.notes.content, lastUpdatedBy: 'Host' });
  pool.query(
    `INSERT INTO meeting_notes (meeting_id, content, updated_by) VALUES ($1, $2, $3)
     ON CONFLICT (meeting_id) DO UPDATE SET content = $2, updated_by = $3, updated_at = NOW()`,
    [req.meeting.id, req.meeting.notes.content, 'Host']
  ).catch(err => console.error('meeting_notes save error:', err.message));
...
app.post('/api/meetings/:meetingId/files', authApiOrSession, (req, res) => {
  ...
  fileUpload.single('file')(req, res, async (err) => {
    ...
    try {
      const { rows } = await pool.query(
        `INSERT INTO meeting_files (meeting_id, participant_name, filename, original_name, size_bytes, mime_type, storage_path)
         VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id, original_name, size_bytes, mime_type, created_at`,
        [meetingId, participantName, req.file.filename, req.file.originalname.slice(0, 255),
         req.file.size, req.file.mimetype, req.file.filename]
      );
      const fileData = rows[0];
      io.to(meetingId).emit('chat:file', {
        fileId: fileData.id, name: fileData.original_name, size: fileData.size_bytes,
        mimeType: fileData.mime_type, sender: participantName, timestamp: Date.now(),
      });
      trackEvent(req.apiUser?.userId || req.session?.userId, null, 'feature.file_share', { size: req.file.size });
      res.status(201).json(fileData);
    } catch (dbErr) {
      console.error('File save error:', dbErr);
...
app.get('/api/meetings/:meetingId/files', authApiOrSession, async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT id, original_name, size_bytes, mime_type, participant_name, created_at FROM meeting_files WHERE meeting_id = $1 ORDER BY created_at',
...
app.get('/api/meetings/files/:fileId/download', authApiOrSession, async (req, res) => {
  try {
    ...
    const filePath = path.join(MEETING_FILES_DIR, rows[0].storage_path);
...
app.get('/api/templates', authApiOrSession, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT id, name, description, settings, is_default, created_at FROM meeting_templates
       WHERE user_id = $1 OR is_default = TRUE ORDER BY is_default DESC, name`,
      [userId || null]
    );
...
app.post('/api/templates', requireUserSession, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO meeting_templates (user_id, name, description, settings)
       VALUES ($1, $2, $3, $4) RETURNING id, name, description, settings, created_at`,
      [req.session.userId, (name + '').slice(0, 100), (description || '').slice(0, 500), JSON.stringify(settings || {})]
    );
    trackEvent(req.session.userId, req.session.companyId, 'feature.meeting_template', { action: 'create' });
    res.status(201).json(rows[0]);
  } catch (err) {
...
app.put('/api/templates/:id', requireUserSession, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE meeting_templates SET name = COALESCE($1, name), description = COALESCE($2, description),
       settings = COALESCE($3, settings) WHERE id = $4 AND user_id = $5 RETURNING id, name, description, settings`,
      [name?.slice(0, 100), description?.slice(0, 500), settings ? JSON.stringify(settings) : null, req.params.id, req.session.userId]
    );
...
app.delete('/api/templates/:id', requireUserSession, async (req, res) => {
  const { rowCount } = await pool.query(
    'DELETE FROM meeting_templates WHERE id = $1 AND user_id = $2', [req.params.id, req.session.userId]
  );
...
app.get('/api/meetings/recurring', authApiOrSession, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT id, title, recurrence, day_of_week, day_of_month, time_utc, timezone, settings,
              meeting_id, is_active, next_occurrence, created_at
       FROM recurring_meetings WHERE user_id = $1 ORDER BY created_at DESC`,
      [userId]
    );
    res.json({ recurringMeetings: rows });
  } catch (err) {
    res.status(500).json({ error: 'Failed to list recurring meetings' });
  }
});
...
app.post('/api/meetings/recurring', authApiOrSession, async (req, res) => {
  ...
  if (!title || !recurrence || !timeUtc) {
    return res.status(400).json({ error: 'title, recurrence, and timeUtc required' });
  }
  const validRecurrence = ['daily', 'weekly', 'biweekly', 'monthly'];
  if (!validRecurrence.includes(recurrence)) {
    return res.status(400).json({ error: `recurrence must be one of: ${validRecurrence.join(', ')}` });
  }
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO recurring_meetings (user_id, company_id, title, recurrence, day_of_week, day_of_month, time_utc, timezone, settings, meeting_id, admin_token, next_occurrence)
       ...
       RETURNING id, title, recurrence, meeting_id, admin_token, next_occurrence`,
      [userId, companyId, (title + '').slice(0, 100), recurrence,
       dayOfWeek ?? null, dayOfMonth ?? null, timeUtc, timezone || 'UTC',
       JSON.stringify(settings || {}), meetingId, adminToken, nextOccurrence]
    );
    trackEvent(userId, companyId, 'feature.recurring_meeting', { recurrence });
    res.status(201).json({ ...rows[0], joinUrl: `/join/${meetingId}` });
  } catch (err) {
    console.error('Recurring meeting create error:', err);
...
app.put('/api/meetings/recurring/:id', authApiOrSession, async (req, res) => {
  const userId = req.apiUser?.userId || req.session?.userId;
  const { title, recurrence, dayOfWeek, dayOfMonth, timeUtc, timezone, settings, isActive } = req.body;
  try {
    const { rows } = await pool.query(
      `UPDATE recurring_meetings SET
        title = COALESCE($1, title), recurrence = COALESCE($2, recurrence),
        day_of_week = COALESCE($3, day_of_week), day_of_month = COALESCE($4, day_of_month),
        time_utc = COALESCE($5, time_utc), timezone = COALESCE($6, timezone),
        settings = COALESCE($7, settings), is_active = COALESCE($8, is_active)
       WHERE id = $9 AND user_id = $10 RETURNING *`,
      [title?.slice(0, 100), recurrence, dayOfWeek, dayOfMonth, timeUtc, timezone,
       settings ? JSON.stringify(settings) : null, isActive, req.params.id, userId]
    );
    if (!rows.length) return res.status(404).json({ error: 'Recurring meeting not found' });
    res.json(rows[0]);
  } catch (err) {
    res.status(500).json({ error: 'Failed to update recurring meeting' });
  }
});
...
app.delete('/api/meetings/recurring/:id', authApiOrSession, async (req, res) => {
  const userId = req.apiUser?.userId || req.session?.userId;
  const { rowCount } = await pool.query(
    'DELETE FROM recurring_meetings WHERE id = $1 AND user_id = $2', [req.params.id, userId]
  );
  if (!rowCount) return res.status(404).json({ error: 'Recurring meeting not found' });
  res.json({ deleted: true });
});
...
const pollLimiter     = createSocketRateLimiter(10, 10000);
const qaLimiter       = createSocketRateLimiter(10, 10000);
const notesLimiter    = createSocketRateLimiter(10, 10000);
...
io.on('connection', (socket) => {
  ...
  socket.on('join-meeting', ({ meetingId, name, isAdmin, adminToken }) => {
    ...
    // Track attendance
    meeting.attendance.push({ participantId, name: safeName, joinedAt: Date.now(), leftAt: null });
  ...
  socket.on('poll:create', ({ question, options }) => {
    if (!pollLimiter.check(socket.id)) return;
    const meeting = meetings.get(currentMeetingId);
    if (!meeting) return;
    const p = meeting.participants.get(currentParticipantId);
    if (!p?.isAdmin) return;
    ...
    const poll = {
      ...
      question: (question + '').trim().slice(0, 500),
      options: options.slice(0, 10).map((o, i) => ({ id: `opt_${i}`, text: (o + '').trim().slice(0, 200) })),
      votes: new Map(), isActive: true, createdBy: p.name, createdAt: Date.now(), showResults: false,
    };
  ...
  socket.on('poll:vote', ({ pollId, optionId }) => {
    ...
    if (!poll.options.find(o => o.id === optionId)) return;
  ...
  socket.on('qa:ask', ({ text }) => {
    if (!qaLimiter.check(socket.id)) return;
    const meeting = meetings.get(currentMeetingId);
    if (!meeting) return;
    const p = meeting.participants.get(currentParticipantId);
    if (!p) return;
    ...
    const questionId = crypto.randomUUID();
    const question = {
      id: questionId, text: text.trim().slice(0, 500),
      askedBy: { participantId: currentParticipantId, name: p.name },
      upvotes: new Set(), isAnswered: false, answer: null, answeredBy: null,
      createdAt: Date.now(), dismissed: false,
    };
    meeting.questions.set(questionId, question);
    io.to(currentMeetingId).emit('qa:new', {
      id: question.id, text: question.text, askedBy: question.askedBy,
      upvoteCount: 0, isAnswered: false, createdAt: question.createdAt,
    });
  ...
  socket.on('qa:upvote', ({ questionId }) => {
    ...
    if (!q) return;
    if (q.upvotes.has(currentParticipantId)) q.upvotes.delete(currentParticipantId);
    else q.upvotes.add(currentParticipantId);
  ...
  socket.on('qa:answer', ({ questionId, answer }) => {
    ...
    if (!q) return;
    q.isAnswered = true;
    q.answer = (answer || '').slice(0, 1000);
    q.answeredBy = p.name;
    io.to(currentMeetingId).emit('qa:updated', {
      questionId: q.id, isAnswered: true, answer: q.answer, answeredBy: q.answeredBy,
    });
  });
  ...
  socket.on('notes:update', ({ content }) => {
    if (!notesLimiter.check(socket.id)) return;
    const meeting = meetings.get(currentMeetingId);
    if (!meeting) return;
    const p = meeting.participants.get(currentParticipantId);
    if (!p?.isAdmin) return;
    ...
    meeting.notes.lastUpdatedBy = p.name;
    meeting.notes.lastUpdatedAt = Date.now();
    socket.to(currentMeetingId).emit('notes:updated', { content: meeting.notes.content, lastUpdatedBy: p.name });
    pool.query(
      `INSERT INTO meeting_notes (meeting_id, content, updated_by) VALUES ($1, $2, $3)
       ON CONFLICT (meeting_id) DO UPDATE SET content = $2, updated_by = $3, updated_at = NOW()`,
      [currentMeetingId, meeting.notes.content, p.name]
    ).catch(err => console.error('meeting_notes save error:', err.message));
  });

  // ─── Breakout Rooms (socket) ──────────────────────────────────────────────────
  socket.on('breakout:create', ({ rooms }) => {
    const meeting = meetings.get(currentMeetingId);
    if (!meeting) return;
    const p = meeting.participants.get(currentParticipantId);
    if (!p?.isAdmin) return;
    if (!Array.isArray(rooms) || rooms.length < 1 || rooms.length > 50) return;
    meeting.breakoutRooms.clear();
    for (const r of rooms.slice(0, 50)) {
      const roomId = crypto.randomUUID();
      meeting.breakoutRooms.set(roomId, {
        id: roomId, name: ((r.name || r) + '').trim().slice(0, 100) || `Room ${meeting.breakoutRooms.size + 1}`,
        participants: new Set(),
      });
    }
    const roomList = [...meeting.breakoutRooms.values()].map(r => ({ id: r.id, name: r.name, participants: [...r.participants] }));
    io.to(currentMeetingId).emit('breakout:updated', { rooms: roomList, isOpen: meeting.breakoutOpen });
    trackEvent(currentUserId, currentCompanyId, 'feature.breakout_rooms', { action: 'create', count: rooms.length });
  });
  ...
  socket.on('breakout:assign', ({ assignments }) => {
    ...
    for (const room of meeting.breakoutRooms.values()) room.participants.clear();
    for (const [roomId, pids] of Object.entries(assignments)) {
  ...
  socket.on('breakout:open', ({ timerMinutes } = {}) => {
    const meeting = meetings.get(currentMeetingId);
    if (!meeting) return;
    const p = meeting.participants.get(currentParticipantId);
    if (!p?.isAdmin) return;
    meeting.breakoutOpen = true;
    const mins = parseInt(timerMinutes) || 0;
    if (mins > 0) {
      if (meeting.breakoutTimer?.timerId) clearTimeout(meeting.breakoutTimer.timerId);
      const endsAt = Date.now() + mins * 60000;
      const mid = currentMeetingId;
      meeting.breakoutTimer = {
        endsAt,
        timerId: setTimeout(() => {
          const m = meetings.get(mid);
          if (!m) return;
    ...
    for (const room of meeting.breakoutRooms.values()) {
      for (const pid of room.participants) {
        const target = meeting.participants.get(pid);
        if (target) io.to(target.socketId).emit('breakout:move', { roomId: room.id, roomName: room.name });
      }
    }
    const roomList = [...meeting.breakoutRooms.values()].map(r => ({ id: r.id, name: r.name, participants: [...r.participants] }));
    io.to(currentMeetingId).emit('breakout:updated', { rooms: roomList, isOpen: true, timerEndsAt: meeting.breakoutTimer?.endsAt || null });
  });
  ...
  socket.on('breakout:close', () => {
    ...
    const p = meeting.participants.get(currentParticipantId);
    if (!p?.isAdmin) return;
    meeting.breakoutOpen = false;
    if (meeting.breakoutTimer?.timerId) clearTimeout(meeting.breakoutTimer.timerId);
    meeting.breakoutTimer = null;
    for (const room of meeting.breakoutRooms.values()) room.participants.clear();
    io.to(currentMeetingId).emit('breakout:closed', { reason: 'Closed by host' });
  });

  socket.on('breakout:broadcast', ({ text }) => {
    const meeting = meetings.get(currentMeetingId);
    if (!meeting) return;
    const p = meeting.participants.get(currentParticipantId);
    if (!p?.isAdmin) return;
    if (!text || typeof text !== 'string') return;
    io.to(currentMeetingId).emit('breakout:message', { text: text.trim().slice(0, 500), from: p.name });
  });
  ...
  socket.on('disconnect', async () => {
    ...
    pollLimiter.cleanup(socket.id);
    qaLimiter.cleanup(socket.id);
    notesLimiter.cleanup(socket.id);
